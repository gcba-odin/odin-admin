...
function ImporterCreateController($scope, $location, usSpinnerService, Alertify, CkanImporterService, rest, model) {
    $scope.model = new model();

    $scope.add = function(isValid) {
        usSpinnerService.spin('spinner');
        if (isValid) {
            var owner = $scope.model.owner;
            var status = $scope.model.status;
            var freq = $scope.model.updateFrequency;
            CkanImporterService.Import(rest, owner, status, freq);
        }
    }
